...
public class Matriks {
    ...
    public void keyboardBiasa(int n, int m){ // modif
        System.out.println();
        for(int i = 1; i <= this.GetBrs(); i++){
            for(int j = 1; j <= this.GetKol(); j++){
    ...
    public void salinMatriksInverse(Matriks Msal){
        for(int i = 1; i <= this.GetBrs()/2; i++){
            for(int j = 1; j <= this.GetKol()/2; j++){
                Msal.SetElmt(i,j,this.GetElmt(i,j));
            }
        }
    }
    ...
    public void hitungJumlah(Matriks M1, Matriks M2){ // modifikasi
        for(int i=1; i<= this.GetBrs(); i++){
            for (int j=1; j<= this.GetKol(); j++){
                this.SetElmt(i, j, M1.GetElmt(i, j) + M2.GetElmt(i, j));
            }
        }
    }

    public void hitungKurang(Matriks M1, Matriks M2){ // modifikasi
        for(int i=1; i<= this.GetBrs(); i++){
            for (int j=1; j<= this.GetKol(); j++){
                this.SetElmt(i, j, M1.GetElmt(i, j) - M2.GetElmt(i, j));
            }
    ...
    public void hitungSkalar(int x){ // modifikasi
        for(int i=1; i<= this.GetBrs(); i++){
            for (int j=1; j<= this.GetKol(); j++){
    ...
    public void hitungKali(Matriks M1, Matriks M2){ // modifikasi
        double jumlah = 0;
        for(int i=1; i<= this.GetBrs(); i++){
            for (int j=1; j<= this.GetKol(); j++){
                for (int k=1; k<= this.GetKol(); k++){
                    jumlah = jumlah + M1.GetElmt(i, k) * M2.GetElmt(k, j);
                }
                this.SetElmt(i, j, jumlah);
                jumlah = 0;
            }
        }
    }

    public void cariTranspose(Matriks M){
        for(int i=1; i<= this.GetBrs(); i++){
            for (int j=1; j<= this.GetKol(); j++){
                this.SetElmt(i, j, M.GetElmt(j, i));
            }
        }
    }

    public void tulisHasilInvers(){
        int mulai = (this.GetKol()/2) + 1;
        for(int i=1; i<= this.GetBrs(); i++){
            for (int j=mulai; j<= this.GetKol(); j++){
